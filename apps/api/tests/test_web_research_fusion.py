...
def test_evidence_fuser_groups_consistent_tempo_and_key_claims():
    ...
    assert profile.music.tempo_bpm == 117.5
    assert profile.music.tempo_confidence > 0.7
...
def test_evidence_fuser_lowers_confidence_for_conflicting_key_claims():
    ...
    assert profile.music is not None
    assert profile.music.key in {"A major", "F# minor"}
    assert profile.music.key_confidence < 0.7
    assert any(note.code == "conflicting_key_claims" for note in profile.music.analysis_notes)
